...
public class V2__import_data extends BaseJavaMigration {
  ...
  public void migrate(Context context) throws Exception {
    ...
    try (InputStream in = getClass().getResourceAsStream("/crashes.csv");
      ...
      for (CSVRecord record : csvRecords) {
        ...
        double milePoint =
            record.get("Mile Point").isEmpty() || record.get("Mile Point").isBlank() ?
        ...
        int laneNumber =
            record.get("Lane Number").isEmpty() || record.get("Lane Number").isBlank() ?
                0 : NumberUtils.createInteger(record.get("Lane Number"));
        String laneType = record.get("Lane Type");
        int numberOfLanes =
            record.get("Number of Lanes").isEmpty() || record.get("Number of Lanes").isBlank() ?
