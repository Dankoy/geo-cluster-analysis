 ...
 * <p>
 * Описание алгоритма: Шаг 1 - первичная инициализация кластеров: 1) Из существующих точек рандомно
 * выбираются центры кластеров. 2) Так же рандомно добавляются точки в кластера
 * <p>
 * Шаг 2 - поиск ближайших точек к центроиду 1) Ищутся ближайшие точки к центру кластера. Рассточние
 * от точки до центра кластера считается по формуле haversine. 2) Центр кластеров пересчитывается
 * <p>
 * Шаг 2 повторяется до тех пор, пока кластера из предыдущей итерации не будут отличатся от текущей
 * итерации.
 * <p>
...
public class KMeansImpl implements KMeans {
  ...
  public List<Cluster> cluster(List<Crash> crashes, int amountOfClusters)
    ...
    return cluster(crashes, amountOfClusters,
        buildRandomInitialClusters(crashes, amountOfClusters));
  }

  /**
   * Uses the KMeans algorithm to generate k clusters from the set of points using a predefined
   * starting set of {@link Cluster}
   *
  ...
  /**
   * Генерирует центры кластеров в рандомных местах, используя существующие координаты точек {@link
   * Cluster}
   *
   * @param crashes          список аварий {@link Crash}
   * @param amountOfClusters количество кластеров
   * @return список кластеров {@link Cluster} содержащий рандомные центры с рандомно распределенные
   * аварии {@link Crash}
   */
  private List<Cluster> buildRandomInitialClusters(List<Crash> crashes, int amountOfClusters) {
    ...
    if (amountOfClusters > crashes.size()) {
      throw new IllegalArgumentException(
          "Expected amount of clusters less or equals amount of accidents, but " +
  ...
  /**
   * Распределяет точки к наиболее подходящему кластеру {@link Cluster}, то есть находит ближайшую
   * точку к кластеру по формуле haversine.
   *
   * @param crashes  список аварий {@link Crash}
  ...
  /**
   * Определяет, если кластера {@link Cluster} сошлись, то есть сравнивается результат предыдущей
   * итерации кластеризации с текущей. Если списки точек {@link Crash} кластеров {@link Cluster} не
   * отличаются друг от друга, то, считаем, что распределили все точки по кластерам корректно.
   *
